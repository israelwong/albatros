...
const HeroSection: React.FC = () => {
    return (
        <section className="relative w-full min-h-[80vh] md:min-h-[70vh] flex flex-col md:flex-row overflow-hidden bg-albatrosBlue"> {/* Quité items-center justify-center del padre para dar más control a los hijos */}

            {/* Contenedor de la Imagen y Overlay (lado izquierdo/superior) */}
            <div className="relative w-full md:w-1/2 h-80 md:h-auto flex-shrink-0"> {/* h-80 para mobile, md:h-auto para desktop, flex-shrink-0 */}
                <Image
                    ...
                    objectFit="cover"
                    priority
                    // Ajustamos la opacidad de la imagen aquí para que no sea tan dominante
                    // y el overlay sea más un efecto de degradado.
                    // Si la imagen es naturalmente oscura, puedes probar con opacity-50 o -60
                    className="z-0 opacity-100 md:opacity-100" // Imagen ligeramente transparente en mobile, opaca en desktop
                />
                ...
                <div className="relative z-20 text-center md:hidden text-white pt-16 pb-6 px-6 flex flex-col justify-end h-full">
                    <h1 className="font-heading text-4xl font-extrabold leading-9 drop-shadow-md">
            ...
            <div className="w-full md:w-1/2 min-h-[40vh] md:min-h-[inherit] flex flex-col items-center md:items-start justify-center text-center md:text-left text-albatrosBlue p-8 md:p-12 md:py-20 flex-grow"> {/* Añadimos flex-grow */}
                ...
                <div className="flex justify-center md:justify-start">
                    <Button
                        onClick={(e) => handleWhatsappClick(e, "Hola, quisiera información sobre las inscripciones para el próximo ciclo. Mensaje desde Hero Section.")}
                        variant="red" size="lg">
                        Asegura tu lugar
                    </Button>
                </div>
